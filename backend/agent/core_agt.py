...
# 我需要你帮我进行后端的代码的更改：
#   1.对上传文件功能进行完善，允许上传多个文件，允许pdf,excel,txt,doc，csv等常用文件上传
#   2.我需要你生成一个新的api:返回我在core_agt里面写的 后台 AI 搜索与网页抓取的内容，你需要严格遵循工业开发后端的api规范
#   3.帮我针对langgraph这个框架的理解，前端要模型在流式输出之前，输出模型调用工具调用sql等等的思考过程，请你组织一下，是通过一个api进行返回还是多个api一起实现？请你决策，最好是
#   多个api实现。
#   4.app.py文件有点臃肿，请你重新重构一下后端的app.py代码，形成多文件也行。
#   5.针对以上内容的完成情况给我生成报告，对每个你实现的功能进行测试。